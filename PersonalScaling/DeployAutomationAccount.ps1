...
param(
	...
	[Parameter(mandatory = $false)]
	[string]$ResourceGroupName = "AVDAutoShutdownDEV",

	[Parameter(mandatory = $false)]
	[string]$AutomationAccountName = "AVDPerShutdownDEV",
	...
	[Parameter(mandatory = $false)]
	[string]$ArtifactsURI = 'https://raw.githubusercontent.com/stephanvandekruis/AVD/blob/main/PersonalScaling'
)
...
$DeploymentStatus = New-AzResourceGroupDeployment -ResourceGroupName $ResourceGroupName -TemplateUri "$ArtifactsURI/AutomationRunbookTemplate.json" -automationAccountName $AutomationAccountName -RunbookName $RunbookName -location $Location -scriptUri "$ScriptURI$($URISuffix)" -Force -Verbose
